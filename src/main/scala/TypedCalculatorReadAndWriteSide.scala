...
object akka_typed {
  ...
  case class TypedCalculatorReadSide(system: ActorSystem[NotUsed])(implicit executionContext: ExecutionContextExecutor) {
    ...
    val graph: Graph[ClosedShape.type, NotUsed] = GraphDSL.create() {
      implicit builder: GraphDSL.Builder[NotUsed] =>
        ...
        val stateUpdater: FlowShape[EventEnvelope, Result] = builder.add(Flow[EventEnvelope].map(e => updateState(e)))
  ...
  object CalculatorRepository {
    ...
    def getLatestsOffsetAndResultSlick(implicit executionContext: ExecutionContextExecutor): Result = {
      ...
      val f: Future[Option[Result]] = db.run(q).map(v => v.flatMap(r => Some(Result(r._1, r._2))))
    ...
    def updatedResultAndOffset(result: Result): Int = {
  ...
  private def updatedResultAndOffsetSlick = {
    (res: Result) => sqlu"update public.result set calculated_value = ${res.state}, write_side_offset = ${res.offset} where id = 1"
  }
